...
public class ElainaEntity extends PathfinderMob implements IAnimatable {
	...
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(ElainaEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(ElainaEntity.class, EntityDataSerializers.STRING);
	...
	public ElainaEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ElainamodModEntities.ELAINA.get(), world);
	}

	public ElainaEntity(EntityType<ElainaEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setPersistenceRequired();
		this.setItemSlot(EquipmentSlot.OFFHAND, new ItemStack(ElainamodModItems.AMNESIAS_SABRE_ITEM.get()));
	}
	...
	public SoundEvent getHurtSound(DamageSource ds) {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("elainamod:hurt"));
	}
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F)) && this.isOnGround() && !this.isVehicle() && !this.isAggressive()) {
			...
			if (this.isVehicle() && event.isMoving()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("ride", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			if (this.isAggressive() && event.isMoving() && !this.isVehicle()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("use_mainhand", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(ElainaEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
